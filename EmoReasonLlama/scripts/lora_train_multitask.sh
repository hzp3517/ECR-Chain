...
deepspeed --include localhost:${gpu_id} --master_port ${MASTER_PORT} EmoReasonLlama/run/train_lora.py \
    ...
    --training_chains_path /data4/hzp/ECR-Chain/chatgpt/prompt_examples/CEE_simplify_fewshot/simplify_subset.json \
    --conv_templ_type "vicuna_v1.1" \
    --main_task_templ "CEE_vanilla" --aux_task_templ_list "CEE_simplify_1shot" --aux_task_distrib_list "1" --aux_task_weight_list "1" \
    --example_id "tr_264_6" \
    --max_remain_utts 20 \
    --bf16 True \
    --remove_unused_columns False \
    --output_dir output/multitask_42 \
    --num_train_epochs 10 \
...
# bash EmoReasonLlama/scripts/lora_train_multitask.sh 6,7